...
//EDIT DATA
if ((isset($_POST['MM_updateEmail'])) && ($_POST['MM_updateEmail'] == "oiaya")) {

    $id_user = $_POST['id_user'];
    $cari_query = sprintf(
        "SELECT id_user FROM user WHERE id_user=%d",
        app($koneksi, $id_user, "int")
    );
    ...
    if ($ResultCari > 0) {
        $updateSQL = sprintf(
            "UPDATE `user` SET `verifikasi_email`=%d WHERE `id_user`=%d",
            app($koneksi, $_POST['verifikasi_email'], "int"),
            app($koneksi, $_POST['id_user'], "int")
        );
        ...
        if ($Result1) {
            $response['kode'] = 1;
            $response['pesan'] = "Verifikasi Email pada Database berhasil";
        }
    } else {
        $response['kode'] = 0;
